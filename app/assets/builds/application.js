...
%o`, message, error2, detail);
  ...
  // node_modules/stimulus-places-autocomplete/dist/stimulus-places-autocomplete.es.js
  ...
  var places_controller_default = class extends src_default {
    connect() {
      super.connect();
  ...
  document.addEventListener("turbo:load", function() {
    ...
    if (menuToggle) {
      menuToggle.onclick = function(e2) {
        e2.preventDefault();
  ...
  document.addEventListener("DOMContentLoaded", async function() {
    ...
    try {
      ...
      console.error("Initializing Card failed", e2);
      return;
    }
    ...
    async function handlePaymentMethodSubmission(event, paymentMethod, shouldVerify = false) {
      ...
      try {
        ...
        const token = await tokenize2(paymentMethod);
        ...
        if (paymentResults && paymentResults.success) {
          window.location.href = `https://www.atiehatie.com/receipt/${paymentResults.payment_id}`;
        } else {
  ...
  async function tokenize2(paymentMethod) {
